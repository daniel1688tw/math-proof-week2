...
### S2_rectify（phase=rectify）
- 這裡 $\delta=\varsigma x^2$ 依賴 $x$，這只保證逐點連續。一致連續要求 $\delta$ 獨立於 $x$。若 $x$ 很小，$\delta$ 會變多大？
...
### S2_rectify（phase=rectify）
- 這個等式 $a=(p+q)/2$ 對任意無理數都成立嗎？試著找一個無理數，它無法寫成兩有理數的平均。
...
### S2_rectify（phase=rectify）
- Heine-Cantor 要求定義域是緊緻空間，$\mathbb{R}$ 不緊緻，故不能直接套用。那 $|f'|\le M$ 的條件究竟起什麼作用？
...
### S2_rectify（phase=rectify）
- $c\in(0,x)$ 是開區間，所以 $c>0$ 而非 $c\ge0$。由 $c>0$ 得 $e^c>1$，那 $e^x-1$ 大於還是等於 $x$？
...
### S2_rectify（phase=rectify）
- 比較判別法要求比較項本身收斂。這裡 $\sum\frac1n$ 發散，所以不能用它來比較。要比較到哪個已知收斂的級數？
