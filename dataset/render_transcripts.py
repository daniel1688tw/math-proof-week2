...
def _turn_meta(t: dict) -> str:
    ...
    bits = []
    for k, label in (("phase", "phase"), ("level", "等級"), ("ladder_idx", "提示梯"),
                     ("walk_idx", "教學步"), ("walk_retry", "重講")):
        v = t.get(k)
        if v is not None:
...
def render(stem: str) -> str:
    ...
    if dialogues:
        ...
        for r in dialogues:
            ...
            md.append(f"- 升級軌跡：提示梯用到 {esc.get('ladder_idx')}、最高等級 "
    ...
    if probes:
        ...
        for r in probes:
            ...
            extra = ""
            if r.get("hint_ladder"):
                extra = "**分級提示梯**：\n" + "\n".join(
                    f"{i + 1}. {h}" for i, h in enumerate(r["hint_ladder"])) + "\n"
            md += _head(r, extra)
    ...
    if multiturn:
        ...
        md.append("檢查的是結構不變式（洩漏／單問句／階段順序／提示梯守恆／收尾／糾錯不中斷），"
                  "不經評審。\n")
        for r in multiturn:
            md.append(f"\n### {r['id']} / {r['lang']}\n")
            md += _head(r)
