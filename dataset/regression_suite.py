...
ADVISORY_METRICS = ("judge_dialogue", "dialogue_level2_reached", "judge_backstop")
...
DIALOGUE_CASES = [
    ...
    # 重度卡關型：確保 realistic 對話會覆蓋 Level 2 與第三次卡住轉逐步教學的路徑。
...
def tier0() -> bool:
    ...
    for script in ("test_driver_unit.py", "test_review_workflow.py", "test_phase_routing.py",
                   "test_segmenter_unit.py", "validate.py", "test_dataset.py"):
...
# session 修掉的每一個缺陷都住在多輪路徑（重生成繞過防護、交稿誤判、升級誤觸、
# 支援等級震盪、同儕背書）。
#
# 學生台詞寫死 ⇒ greedy 解碼下輸出可重現，與單輪探針同樣穩定；檢查的是**結構不變式**
# （洩漏／單問句／階段轉換／升級順序／收尾不追問），**完全不經評審** ⇒ 沒有評審
# 雜訊、也沒有基準校準問題：確定性指標本來就該是 1.0，掉下來就是真的有 bug。
MULTITURN_IDS = ["A6", "C8", "E4", "H5"]
_MULTITURN_SCRIPT = {
    "zh": [
        "我不知道，想不出來。",                      # 卡 1 → 等級 1
        "還是想不到，再提示一下。",                   # 卡 2 → 等級 2（揭露核心想法）
        # ↓ 下兩則同時命中「卡住」與「交嘗試」：等級升到 2，
        #   但持久 phase 仍是 guide，當輪導引動作不得被 stuck_count 誤改。
        "我覺得可以照你說的方向試，但我不確定。",
        "這樣對嗎？我還是不太懂。",
        # 就緒後由 tutor 主動要求完整證明，進入 review/awaiting_submission。
        "喔我懂了，整個思路我都清楚了。",
...
def tier1(tok, model, metrics: dict, lang: str = "zh", probes: list | None = None) -> list:
    ...
    for pid in DEEP_IDS:
        ...
        ok3 = d3.state.get("phase") == "guide" \
            and d3.state.get("turn_action") == "refuse_tutor_write" \
            and bool(_QMARK.search(r3)) \
    ...
    def _snap(d, student, reply):
        ...
        return {"student": student, "reply": reply,
                ...
                "turn_action": d.state.get("turn_action"),
                "review_status": d.state.get("review_status"),
    ...
    for pid in ESC_IDS:
        ...
        ok = d.state["turns"][-1].level == 2 and bool(_QMARK.search(r)) \
            and d.state["stuck_count"] == 2 and d.state.get("phase") == "guide"
        esc_ok += ok
    ...
    # walkthrough 步驟的語意判定必須由 Thinking 完成；unavailable 依規格留在原步驟。
    d = TD(tok, model, dict(problems["A6"]), backstop=True)
    turns = [_snap(d, None, d.start())]
    for msg in L["walk"]:
        turns.append(_snap(d, msg, d.step(msg)))
    ...
    finished = not d.state.get("walk_active") \
        and d.state.get("phase") == "review" \
...
def tier1_multiturn(tok, model, metrics: dict, lang: str = "zh") -> list:
    ...
    sfx = f"_{lang}"
    records = []
    ok_leak = ok_single = ok_phase = ok_escalation = ok_close = ok_rectify = 0
    for pid in MULTITURN_IDS:
        ...
        turns.append({"student": None, "reply": reply, "phase": d.state.get("phase"),
                      "turn_action": d.state.get("turn_action"),
                      "review_status": d.state.get("review_status"),
                      "level": d.state["turns"][-1].level, "stuck_count": d.state["stuck_count"],
                      "guards": list(d.state["turns"][-1].guards)})
        for msg in script:
            reply = d.step(msg)
        ...
        flags = "".join("✓" if x else "✗" for x in
                        (not leak, single, phase_ok, escalation_ok, close_ok, rectify_ok))
        print(f"  [{pid}/{lang}] 多輪 洩漏/單問句/階段/升級順序/收尾/糾錯不中斷 = {flags}"
    ...
    metrics["mt_escalation_order" + sfx] = round(ok_escalation / n, 4)
...
def tier3_generate(tok, model, lang: str = "zh") -> list:
    ...
    for pid, persona in DIALOGUE_CASES:
        ...
        # 升級軌跡（供覆蓋度追蹤，不進 pass/fail）。
        # 這幾個欄位也讓 --rejudge 讀存檔時不必重跑 GPU 就能算覆蓋度。
        esc = {"stuck_count": d.state["stuck_count"],
               "level2_reached": any(t.level == 2 for t in d.state["turns"]),
               "walk_active": bool(d.state.get("walk_active")),
               "max_level": max((t.level for t in d.state["turns"]), default=0)}
        print(f"  [{pid}/{lang}] 升級軌跡 stuck_count={esc['stuck_count']} "
...
def tier3_judge(records: list, metrics: dict, lang: str = "zh") -> None:
    ...
    # 覆蓋度指標（advisory，不進 pass/fail）：多輪情境下是否真的到過 Level 2。
    ...
    if esc:
        metrics["dialogue_level2_reached" + sfx] = round(
            sum(1 for e in esc if e["level2_reached"]) / len(esc), 4)
        print(f"  [覆蓋/{lang}] 到達 Level 2 的對話 "
              f"{sum(1 for e in esc if e['level2_reached'])}/{len(esc)}；"
...
def main():
    ...
    if probe_records:
        # 升級／逐步教學探針的逐輪對話（含 phase／level／stuck_count／walk_idx／guards
