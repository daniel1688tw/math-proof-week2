...
def assemble_problem(statement: str, result: dict, pid: str = "USER") -> dict:
    ...
    if result["status"] == "verified":
        ...
        # 教學步驟的語言標記：driver 的逐步教學要靠它決定保底問句與答案鍵的語言
        prob["teach_steps_lang"] = result.get("teach_steps_lang", "zh")
        prob["teach_steps_source"] = result.get("teach_steps_source")
        prob["teach_steps_initial_status"] = result.get("teach_steps_initial_status")
        for lang in ("zh", "en"):
            if result.get(f"teach_steps_{lang}"):
                prob[f"teach_steps_{lang}"] = result[f"teach_steps_{lang}"]
                prob[f"teach_steps_source_{lang}"] = result.get(
                    f"teach_steps_source_{lang}")
    return prob
