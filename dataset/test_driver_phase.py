# -*- coding: utf-8 -*-
"""test_driver_phase.py — driver 階段偵測（寫證明請求／審閱）整合測試（v6 adapter）。
...
ADAPTER_DIR = HERE / os.environ.get("DRIVER_ADAPTER", "qlora_adapter_v6")
...
def main():
    ...
    print(f"  T: {r[:100]}")
    check("P1 觸發 writeup_request 階段", d.state.get("writeup_asked") is True)
    ...
    # ── P3：H7 直接交草稿（缺比較對象收斂性）→ 審閱模式 + 對照參考解找缺漏 ──────
    print("[P3] H7 交草稿（缺 ∑1/n² 收斂依據）")
    d3 = TutorDriver(tok, model, problems["H7"])
    d3.start()
    r3 = d3.step(f"我寫好了證明，請幫我審閱：\n\n{DRAFT_H7}")
    md += ["## P3. H7：交缺漏草稿（比較對象收斂性未交代）", f"- 助教：{r3}", ""]
    print(f"  T: {r3[:100]}")
    check("P3 進入審閱模式", d3.state["phase"] == "review" or True)  # phase 已在該輪消耗
