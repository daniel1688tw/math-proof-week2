# 評審後端遷移：Antigravity CLI（`agy` / **Gemini 3.6 Flash Medium**）

**狀態：已完整接入並採用（2026-07-22）。預設 `JUDGE_BACKEND=antigravity` +
`AGY_MODEL="Gemini 3.6 Flash (Medium)"`；Claude 保留為 `JUDGE_BACKEND=claude` 備援。**
見 §七選型結論。

> **§七 最終選型結論（2026-07-22，四個檔位、多輪對同一 v9 模型實測）**
>
> 三檔位各 25 次穩定性壓測皆 100% 可解析、零逾時。差異在**判準對照**與**對話層可重現性**：
>
> | 檔位 | 判準對照(錯誤/乾淨) | zh 對話數學兩輪 | 引導/數學區分 | 判定 |
> |---|---|---|---|---|
> | 3.5 Flash Medium | ✓ / ✗ 憑空捏造誤殺 | — | — | 淘汰 |
> | 3.1 Pro Low/High | ✓ / ✗ 過嚴誤殺 | **0.333↔0.0 崩潰** | ❌ 混淆 | 淘汰 |
> | **3.6 Flash Medium** | **✓ / ✓** | **0.667↔0.667 逐字一致** | ✅ 正確分離 | **採用** |
>
> **判斷**：3.6 Flash Medium 是唯一同時通過「判準對照 2/2」「zh 對話數學兩輪穩定」
> 「正確區分引導問題與數學錯誤」的檔位，且最快（6.5s）、最省額度。3.1 Pro 反而因
> 對話層崩潰＋把引導誤判成數學錯而淘汰（模型層級高 ≠ 當裁判可靠）。
>
> **殘餘**：en 對話層與後盾（judge_backstop）仍有 ±1 案的 n=3 擺動
> （dialogue_math_ok_en 1.0↔0.667、backstop 0.667↔0.5），這是 3 樣本 + LLM 即興學生
> 的本質雜訊，Claude 也有；基準取**兩輪保守 min** 吸收（見 `regression_baseline_antigravity.json`
> 的 `note`），不會誤殺。
>
> **決策**：**預設採用 3.6 Flash Medium**。達成脫離 Claude 額度依賴的目標，且對話層
> （抓 v9/v10 退步最關鍵處）判準穩定可靠。Claude 保留為備援（`JUDGE_BACKEND=claude`）。
