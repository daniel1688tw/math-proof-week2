...
week3/
├── dataset/                          # ★ 一切核心
│   ├── src/                          # 手寫內容源碼（50 題 + 400 對話，Codex 撰寫並驗證）
...
│   ├── regression_suite.py           # ★ 推送前守門（Codex 當評審+學生；退步即 exit 1）
...
├── .Codex/skills/pre-push-check/    # /pre-push-check skill：推送前守門流程（進版控）
...
python dataset\regression_suite.py             # 完整：Codex 當評審+學生（GPU+Codex CLI，~1.5hr）
...
`--rejudge` 補評即可（auto_gate 會自動記進度接續，Codex Pro 額度中斷不會賠掉整輪）。
...
- **限流防呆**：`claude_call` 把「You've hit your limit」等限額訊息當可重試失敗——
  ...
  `JUDGE_BACKEND=Codex` 才成立**（2026-07-22 起預設後端改為 `antigravity`＝agy／
  Gemini 3.6 Flash Medium，學生與評審皆走 Gemini、不碰 Codex 額度，本 session 可照常工作，
  見 v11 迭代 2026-07-24）。若手動切回 `JUDGE_BACKEND=Codex` 則舊規則恢復（同帳號搶額度）。
...
- **reveal_ok_zh 加雜訊容忍 ε=0.08**（與 s2_catch 同型：確定性生成、Codex 評審、n≈13）：
...
- Antigravity CLI（`agy`）已完整接入 `regression_suite.py` 抽象層並成為**預設評審**
  ...
  Codex 保留為 `JUDGE_BACKEND=Codex` 備援。達成脫離 Codex 額度依賴。
...
- 基準 `regression_baseline_antigravity.json` = 兩輪保守 **min**（吸收 en 對話/後盾 ±1 案
  的 n=3 本質雜訊，Codex 亦有）。不同裁判的尺不可互比，故獨立基準檔。
...
7. ~~S4 僅 3 案例~~ → 已擴到 6 案例（2026-07-16）。**教訓：輸入隨機的指標（S4 學生方案、
   Tier 3 對話由 Codex 即興生成）基準不可棘輪到幸運最大值**，應設在可辯護的真實水準
