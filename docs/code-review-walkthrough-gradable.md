...
> **歷史文件／已被取代：**本文件以下記錄的是舊版「答案字串比對＋錯答重試」設計的
> code review。現行實作已刪除 `grade_walkthrough_answer`、`walk_retry` 執行路徑、
> `_MAX_WALK_RETRY` 與重講模型守衛。每個確認問題只作答一次，回答交給與
> Review／Rectify 相同的思考型審閱後盾做數學語意判定；非 `correct` 或後盾不可用時，
> 顯示參考答案並前進。以下內容只保留作為問題沿革，不應當作現行規格。
