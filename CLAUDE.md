...
17. **學生持續卡關時助教退化成重複迴圈**（2026-08-02，「重度卡關型」persona 首跑即抓到，
    中英 `guidance` 皆 1 分、`math_ok=True` ＝純教學失敗）。兩個成因皆已定位且有生產證據：
    (a) driver 補的保底句把「逐字重複」的 difflib 相似度稀釋到 0.85 門檻以下（實測 0.768）
    → `repeat` 根本不觸發；(b) 真觸發時 `_regen` 回傳的仍是同一段文字，程式碼**不複驗**
    就採用。打擊面正是提示梯用盡、最需要幫助的學生。**下輪最高優先**：比對前剝除
    driver 自己補的保底句／nudge；重生成後複驗，仍重複就換措辭或推進 `walk_idx`。
