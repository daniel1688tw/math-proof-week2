...
5. **不順學生的替代證法** → v8 補 altmethod/ontrack 訓練樣本後**中文已根治**（S4 6/6 穩定），
   **英文有殘留**（四輪抽樣 1.0/0.833/1.0/0.667，真實水準 ~0.85；典型失手：X4「你的反證可行，
   但標準方法更乾淨」拉回參考解、H3 把學生的 ε-δ 誤稱為參考解的序列法）。v9 再補英文樣本。
6. ~~S4 僅 3 案例~~ → 已擴到 6 案例（2026-07-16）。**教訓：輸入隨機的指標（S4 學生方案、
   Tier 3 對話由 Claude 即興生成）基準不可棘輪到幸運最大值**，應設在可辯護的真實水準
   （altmethod_en 基準 0.8333 = 容一案），否則正常跑分必然誤判退步。
7. 學生說「懂了、謝謝」後助教偶有多講一段延伸講解（對話評審扣分項）；`_DONE_RE` 蓋不住
   「講解型」延伸（只擋得住追問型），需下輪處理。
