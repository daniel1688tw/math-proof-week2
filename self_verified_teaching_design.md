...
> 純推論端改動，不重訓模型。
>
> **2026-08-31 現況補記：**本文件保留原始設計脈絡；現行四 phase、Level 2 微支架與
> unavailable 行為以 `docs/code-review-level-phase-2026-08-31.md` 為準。
...
進入前的三層支援：Level 0 只問一個聚焦問題；Level 1 把同一缺口縮成更小的子問題；
Level 2 給一個可執行微支架，可含一個必要公式，但不可繼續代寫後續推導。第三次仍卡住才切換。
...
每輪：確定性輸出「第 i/n 步：{explain}\n\n確認問題：{check}」（不呼叫生成模型），
  ...
  下一輪 judge_walkthrough_answer(...)：
    ...
    後盾離線、逾時或格式無法解析 → 不猜答案是否正確；保留原步驟並請學生稍後重答
...
- **後盾故障採 fail-closed 學習進度**：若審閱服務不可用，不以字串猜測，也不把失敗
  當成學生完成該步的證據；保留 `walk_idx` 與已呈現步驟，待服務恢復後重試。
  判定狀態記在 `walkthrough_review_status` 供除錯。
