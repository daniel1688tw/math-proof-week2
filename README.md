...
本專案微調了 **Qwen3-4B-Instruct** 模型，建構出一個蘇格拉底式的大學微積分／數學分析證明助教系統。該系統同時支持「中英雙語」引導與動態語言偵測：
...
訓練資料**全部人工撰寫並驗證數學正確性**（50 道題目 + 400 條繁體中文與雙語對話），
在單張 6GB VRAM 的筆電 GPU（RTX 4050）上即可完成訓練與推論。

## 架構：模型只管數學與語氣，決策交給程式碼

本專案的詳細雙層架構設計、TutorDriver 狀態機控制流、QLoRA 微調細節與檔案對照表，請參閱 [ARCHITECTURE.md](file:///d:/UserData/claude_project/霓資料/math-proof-week2/ARCHITECTURE.md)。

本專案採用**雙層混合架構**：
1. **程式驅動決策（TutorDriver）**：負責控制對話狀態機、提示等級（stuck counter）與後處理（單問句截斷與防洩漏檢查），以確保對話流程與邏輯受控。
2. **微調模型（QLoRA）**：專注於數學證明推導的正確性，並以蘇格拉底式的語氣進行引導與解答。
