...
- **分級提示**——Level 0 聚焦、Level 1 縮小子問題、Level 2 給一個可執行微支架；第三次連續卡住才進逐步教學
- **自我驗證備課**——面對沒見過的題目，先自己證一遍並獨立驗證，證對了才教；證不出來誠實說沒把握
- **寫證明與審閱**——完整證明先做兩輪全文審閱；所有問題先保存、每輪只處理一項，
  局部訂正通過語意複核後才合併，最後再審閱乾淨完整證明
...
```
新題目 → auto_reference.py 備課（qwen3-4b-thinking-2507）
          PROVER×3 → VERIFIER → 必要時 REPAIR／再驗證 → SEGMENTER（最多三次）
          verified → 產出 reference_proof + teach_steps，進入下方 grounded 教學
          unverified → 同學模式（誠實聲明沒把握、同儕一起探索、被質疑會反省認錯）

學生訊息 → TutorDriver（確定性決策層）
             · stuck_count：0 正常引導／1 縮小同一缺口／2 給一個可執行微支架／3 進逐步教學
             · 持久 phase 只有 guide / walkthrough / review / closed；逼問、嘗試、釐清等是 turn_action
             · phase 只接受 Controller event；學生宣告與 Tutor 候選文字都不能自行切 phase
             · Level 2 可包含一個定理、技巧、輔助構造或必要公式，但不得越過該支架代寫後續推導
             · 第三次連續卡住 → walkthrough 逐步教學：**確定性模板、不呼叫說話模型**，
               一步一確認；學生只有一次有效作答機會，錯誤／不完整時揭答並前進；
               審閱不可用時保留原步驟，不判錯、不揭答也不推進；
               回答採與 review/rectify 相同的思考型語意審閱，不做字串比對；教完仍要學生自寫證明
           ...
           → 內容防護（每次重生成後重跑）：參考解洩漏 n-gram 檢查、on-track 防奉送、
             Level 2 單一微支架語意檢查、稱讚校準；其後單問句截斷、重複偵測、回問保底
```

核心設計原則：**離散決策交給程式碼、數學內容先驗證、模型只負責數學與語氣**。
教學深度由 `stuck_count` 決定；phase、turn action 與 level 是三條正交控制線。
...
`qwen3-4b-thinking-2507`）。服務不可用時系統不崩潰，但需要可靠判定的 walkthrough／review
會保守停留、等待重試，不會假裝學生答對或證明已通過。

最新 level／phase code review 與待修項目見
[docs/code-review-level-phase-2026-08-31.md](docs/code-review-level-phase-2026-08-31.md)。
...
| Tier 1b | **多輪確定性探針**（`mt_*`：多輪不洩漏、階段流、升級順序、乾淨收尾） | 零容忍 |
...
> 現場備課產出的參考解與教學步驟。推論時只載入題目、參考解與已驗證教學步驟。
>
