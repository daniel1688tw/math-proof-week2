---
name: pre-push-check
description: 推送 GitHub 前的品質守門：跑全自動回歸套件（Codex 當評審與學生）、比對基準、檢查推送範圍，給出「可推 / 不可推」判定。使用者說「推之前先測」「跑回歸」「pre-push」時使用。
---
...
- `Codex --version` 可用（Tier 2/3 評審與學生角色必需）
...
另外必做的質性抽查（不只看數字）：
- 打開 `*_dialogues.json`，親自讀 3 場 Codex-學生對話全文，確認助教沒有
...
- 套件設計說明在 `dataset/regression_suite.py` docstring；指標定義：
  ...
  ＋ Codex 評審（judge_math_ok / judge_s2_catch / judge_score /
